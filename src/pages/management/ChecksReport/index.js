...
import {GET_CECKS_REPORTS} from 'query/orderReoprt/getOrder';
...
const ChecksReports = () => {
  ...
  const [getReports, {data}] = useLazyQuery(GET_CECKS_REPORTS);
  ...
  const [reportLanguage, setReportLanguage] = useState('ar'); // State for report language selection
  ...
  const handleSubmit = () => {
    ...
    const variables = {
      ...
      checkType, // Include the checkType in your query variables
    };
    ...
    getReports({
      ...
      .then((response) => {
        const base64Excel = response.data.getCheckReports.downloadUrl;
    ...
    <Card sx={{m: 3}}>
      <CardContent>
        ...
        <ButtonGroup
          sx={{mt: 2}}
          variant='outlined'
          aria-label='outlined button group for check type'
        >
          <Button
            sx={buttonStyle(checkType === 'Checks')}
            onClick={() => handleCheckTypeChange('Checks')}
          >
            {messages['Checks']}
          </Button>
          <Button
            sx={buttonStyle(checkType === 'issuedChecks')}
            onClick={() => handleCheckTypeChange('issuedChecks')}
          >
            {messages['sidebar.finance.issuedChecks']}
          </Button>
        </ButtonGroup>
        ...
        <FormControl fullWidth margin='normal'>
          ...
          >
            {checkType=="issuedChecks"?
            <MenuItem value='cashed'>{messages['chasedChecks']}</MenuItem>
            :""}
