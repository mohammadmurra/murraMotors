...
const CheckDetails = () => {
  ...
  const {checkId, debtorId} = useParams();
  ...
  const handleUpdateCheck = async () => {
    try {
      ...
      let finalValue = editedCheck.value;
      // Check if the currency is USD and a conversion rate is provided
      if (editedCheck.currency === 'USD' && editedCheck.conversionRate) {
        // Convert the value from USD to ILS
        finalValue = editedCheck.value * parseFloat(editedCheck.conversionRate);
      }

      if (editedCheck.cashed) {
        // Proceed with the deletion of the check from the 'Checks' collection
        await db.collection('Checks').doc(checkId).delete();

        const debtorDoc = await db.collection('Debtors').doc(debtorId).get();
        if (debtorDoc.exists) {
          // Prepare the payment entry with the converted value if necessary
          const paymentEntry = {
            id: checkId,
            date: editedCheck.date,
            note: editedCheck.notes || '',
            type: 'check',
            value: finalValue, // Use the possibly converted value here
            currency: 'ILS',
          };

          // Update the debtor's document with the new payment entry
          await db
            .collection('Debtors')
            .doc(debtorId)
            .update({
              checks: firebase.firestore.FieldValue.arrayRemove(checkId),
              payments: firebase.firestore.FieldValue.arrayUnion(paymentEntry),
            });
        }
      } else {
        // If not cashed, just update the check details without conversion
        await db.collection('Checks').doc(checkId).update(editedCheck);
      }
  ...
  const handleButtonClick = () => {
    if (editMode) {
      if (editedCheck.cashed && editedCheck.currency === 'USD') {
        const rate = parseFloat(editedCheck.conversionRate || 0);
        const finalValue = editedCheck.value * rate;
        setConversionDetails({
          conversionRate: rate,
  ...
  const handleDeleteCheck = async () => {
    try {
      ...
      // First, remove the check from the 'checks' array of the debtor
      const debtorRef = firebase
        .firestore()
        .collection('Debtors')
        .doc(debtorId);
      ...
      await firebase.firestore().collection('Checks').doc(checkId).delete();
